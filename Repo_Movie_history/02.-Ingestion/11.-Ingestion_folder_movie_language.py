...
# MAGIC %md
# MAGIC # Ingestion de archivos "movie_languaje_*.json"
...
path_movie_language = f"{bronze_folder_path}/{v_file_date}/movie_language/"
...
movie_language_final_df.write.mode("append").partitionBy("file_date").format("parquet").saveAsTable("movie_silver.movie_language")
...
# MAGIC %sql
# MAGIC SELECT * FROM movie_silver.movie_language WHERE movie_id is not null
...
# MAGIC %fs
# MAGIC ls /mnt/moviehistorico/silver/movie_language

# COMMAND ----------

# consulto parquet en ruta de capa silver con estructura de tabla
display(spark.read.parquet("/mnt/moviehistorico/silver/movie_language"))
