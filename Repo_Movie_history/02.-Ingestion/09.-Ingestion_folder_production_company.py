...
# MAGIC %md
# MAGIC # Ingestion de archivos "production_company_*.csv"
...
path_production_company = f"{bronze_folder_path}/{v_file_date}/production_company/"
...
production_company_df = add_ingestion_date(production_company_df) \
...
spark.sql("DROP TABLE IF EXISTS movie_silver.production_company")
...
# MAGIC %sql
# MAGIC SELECT * FROM movie_silver.production_company;

# COMMAND ----------

# transifero el df final a la ruta de la capa silver en formato parquet.
## -- production_company_final_df.write.mode("overwrite").parquet(f"{silver_folder_path}/production_company")

# COMMAND ----------

# MAGIC %fs
# MAGIC ls /mnt/moviehistorico/silver/production_company

# COMMAND ----------

# consulto parquet en ruta de capa silver con estructura de tabla
display(spark.read.parquet("/mnt/moviehistorico/silver/production_company"))
